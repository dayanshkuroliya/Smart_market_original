...
// 1. 'Not Paid' ko hata kar 'Absent' kar diya
const STATUS_OPTIONS = ['Paid', 'Pending', 'Absent']

// 2. Color styling badal di: Absent ko purple/gray tint de diya hai
const statusColor = {
  'Paid':     'border-green-400 bg-green-50 dark:bg-green-900/20',
  'Pending':  'border-yellow-400 bg-yellow-50 dark:bg-yellow-900/20',
  'Absent':   'border-purple-400 bg-purple-50 dark:bg-purple-900/20', 
}
...
export default function DailyCollection() {
  ...
  // Summary structure me absent count add kar diya
  const [summary, setSummary] = useState({ collected: 0, pending: 0, paid: 0, absent: 0 })
  ...
  // 3. Recompute summary logice me 'Not Paid' ki jagah 'Absent' lagaya
  useEffect(() => {
    let collected = 0, pending = 0, paid = 0, absent = 0
    vendors.forEach(v => {
    ...
    setSummary({ collected, pending, paid, absent })
  }, [collections, vendors])
    ...
    <div className="space-y-4">
      ...
      <div className="card p-4 flex flex-wrap items-center gap-4">
        ...
        <div className="flex gap-4 text-sm flex-wrap">
          ...
          <span className="text-purple-600 dark:text-purple-400 font-semibold"> Absent: {summary.absent}</span>
                          ...
                          {STATUS_OPTIONS.map(s => (
                            <button
                              ...
                              className={`text-xs px-2.5 py-1 rounded-lg font-medium transition-all
                                ${col?.status === s
                                  ? s === 'Paid' ? 'bg-green-500 text-white' : s === 'Pending' ? 'bg-yellow-400 text-white' : 'bg-purple-500 text-white'
